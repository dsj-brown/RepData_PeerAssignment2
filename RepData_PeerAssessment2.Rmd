...
```
The NOAA storm database is an official publication of the National Oceanic and Atmospheric Administration (NOAA).  It holds data relating to storm events which have been registered between `r format(min(sData$BGN_DATE, na.rm = TRUE),"%B %Y")` and `r format(max(sData$END_DATE, na.rm = TRUE),"%B %Y")`.  
  
This analysis focuses upon the most serious recorded storm events in relation to their population health and economic consequences.  
...
* The most economic damage is caused by the event **`r head(mostEcon$EVTYPE, 1)`**, which accounts for `r sprintf("%.1f%%", 100 * head(mostEcon$pc, 1))` of all recorded damage (`r dollar(round(head(mostEcon$tEcon, 1),1), big.mark = ",")` billion).  
...
Convert the damage values so they all take the multipliers into consideration and resolve to a $million value.  
  
```{r updateValues}
upVal <- data.frame(PROPDMGEXP = c("K", "M", "B"), multP = c(1000, 1000000, 1000000000))
sData <- merge(sData, upVal, by = "PROPDMGEXP")
sData$PROPDMG_M <- (sData$PROPDMG * sData$multP)/1000000000

upVal <- data.frame(CROPDMGEXP = c("K", "M", "B"), multC = c(1000, 1000000, 1000000000))
...
Where a multiplier is available it has been used to derive cost in billions  Property and crop damage has been added together to derive total damage. No adjustments have been made for inflation.
